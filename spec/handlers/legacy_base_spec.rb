...
describe YARD::Handlers::Ruby::Legacy::Base, "#tokval" do 
  ...
  it "should not allow interpolated strings with TkSTRING" do
    tokval('"#{c}"', Parser::Ruby::Legacy::RubyToken::TkSTRING).should be_nil
  end
  ...
  it "should return nil for any non accepted type" do
    ...
    tokval(':sym', Parser::Ruby::Legacy::RubyToken::TkId).should be_nil
  end
  ...
  it "should accept any ID type if TkId is set" do
    tokval('variable', Parser::Ruby::Legacy::RubyToken::TkId).should == "variable"
    tokval('CONSTANT', Parser::Ruby::Legacy::RubyToken::TkId).should == "CONSTANT"
  end
  
  it "should allow extra token types to be accepted" do 
    tokval('2.5', Parser::Ruby::Legacy::RubyToken::TkFLOAT).should == 2.5
    tokval('2', Parser::Ruby::Legacy::RubyToken::TkFLOAT).should be_nil
    tokval(':symbol', Parser::Ruby::Legacy::RubyToken::TkFLOAT).should be_nil
  end
...
describe YARD::Handlers::Base, "#tokval_list" do 
  ...
  it "should return the list of tokvalues" do
    ...
    tokval_list(":a, :b, File.read(\"\#{c}\", ['w']), :d", 
      Parser::Ruby::Legacy::RubyToken::Token).should  == [:a, :b, 'File.read("#{c}", [\'w\'])', :d]
  end
  ...
  it "should try to skip any invalid tokens" do
    ...
    tokval_list("CONST1, identifier, File.read(\"\#{c}\", 'w', File.open { }), CONST2", 
      Parser::Ruby::Legacy::RubyToken::TkId).should  == ['CONST1', 'identifier', 'CONST2']
  end
  
  it "should ignore a token if another invalid token is read before a comma" do
    tokval_list(":a, :b XYZ, :c", Parser::Ruby::Legacy::RubyToken::TkSYMBOL).should == [:a, :c]
  end
  
  it "should stop on most keywords" do
    tokval_list(':a rescue :x == 5', Parser::Ruby::Legacy::RubyToken::Token).should == [:a]
  end
  ...
  it "should ignore parentheses around items in a list" do
    ...
    tokval_list(':a, ([:f]), :c)', Parser::Ruby::Legacy::RubyToken::Token).should == [:a, '[:f]', :c]
  end
  ...
  it "should not stop on a true/false/self keyword (cannot handle nil)" do
    tokval_list(':a, true, :b, self, false, :c, nil, File, super, if, XYZ', 
      Parser::Ruby::Legacy::RubyToken::Token).should == [:a, true, :b, 'self', false, :c, 'File', 'super']
  end
