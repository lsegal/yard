...
describe YARD::CLI::Yardoc do
  ...
  describe 'Extra file arguments' do
    shared_context "mocking extra file args" do
      let(:extra_files) { "#{extra1} #{extra2}" }
      let(:globs) { [
        {file: 'README*', retval: [] }
      ] }
      before do
        ...
        yardoc.parse_arguments *"file1 file2 - #{extra_files}".split(/\s/)
      end
    end
    shared_context "options' files" do
      let(:expected) {
        [
          CodeObjects::ExtraFileObject.new(extra1, ''),
          CodeObjects::ExtraFileObject.new(extra2, '')
        ]
      }
      subject { yardoc.options }
    end

    context "That have different base names" do
      context "and no path" do
        let(:extra1) { 'extra_file1' }
        let(:extra2) { 'extra_file2' }
        include_context "mocking extra file args"
        describe "files" do
          subject { yardoc.files }
          it { should == %w( file1 file2 ) }
        end
        include_context "options' files"
        its(:files) { should =~ expected}
      end
    end

    context "extra files that have the same basename" do
      context "but different path" do
        let(:extra1) { 'lib/dir1/extra_file' }
        let(:extra2) { 'lib/dir2/extra_file' }
        include_context "mocking extra file args"
        describe "files" do
          subject { yardoc.files }
          it { should == %w( file1 file2 ) }
    ...
    describe "globs as extra files" do
      let(:extra1) { 'a.txt' }
      let(:extra2) { 'b.txt' }
      let(:globs) { [
        {file: 'README*', retval: [] },
        {file: '*.txt', retval: [extra1, extra2] }
      ] }
      let(:extra_files) { "*.txt" }
      include_context "mocking extra file args"
      include_context "options' files"
