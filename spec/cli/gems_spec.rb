...
describe YARD::CLI::Gems do
  ...
  describe "#run" do
    it "builds all gem indexes if no gem is specified" do
      ...
      expect(Gem::Specification).to receive(:find_all_by_name).with('').and_return([@gem1, @gem2])
      CLI::Gems.run
    end

    it "allows gem to be specified" do
      build_specs(@gem1)
      expect(Gem::Specification).to receive(:find_all_by_name).with(@gem1.name, '>= 0').and_return([@gem1])
      CLI::Gems.run(@gem1.name)
    end

    it "allows multiple gems to be specified for building" do
      build_specs(@gem1, @gem2)
      expect(Gem::Specification).to receive(:find_all_by_name).with(@gem1.name, @gem1.version).and_return([@gem1])
      expect(Gem::Specification).to receive(:find_all_by_name).with(@gem2.name, '>= 0').and_return([@gem2])
      CLI::Gems.run(@gem1.name, @gem1.version, @gem2.name)
    end

    it "allows version to be specified with gem" do
      build_specs(@gem1)
      expect(Gem::Specification).to receive(:find_all_by_name).with(@gem1.name, '>= 1.0').and_return([@gem1])
      CLI::Gems.run(@gem1.name, '>= 1.0')
    end

    it "warns if one of the gems is not found, but it should process others" do
      build_specs(@gem2)
      expect(Gem::Specification).to receive(:find_all_by_name).with(@gem1.name, '>= 2.0').and_return([])
      expect(Gem::Specification).to receive(:find_all_by_name).with(@gem2.name, '>= 0').and_return([@gem2])
    ...
    it "accepts --rebuild" do
      @rebuild = true
      build_specs(@gem1)
      expect(Gem::Specification).to receive(:find_all_by_name).with('').and_return([@gem1])
