...
describe YARD::Parser::SourceParser do
  ...
  describe '#parse_statements' do
    ...
    if YARD.ruby2?
      ...
      it 'supports various components of a method definition combined' do
    ...
    if YARD.ruby21? && !LEGACY_PARSER
      it 'supports named arguments without default values' do
        YARD.parse_string 'def foo(a:, b:, **kwargs) end'
        args = [['a:', nil], ['b:', nil], ['**kwargs', nil]]
        Registry.at('#foo').parameters.should eq(args)
      end
